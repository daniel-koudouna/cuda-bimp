  ...
  namespace utils {
    ...
    void showTimes() {
      for (auto it = timeMap.begin(); it != timeMap.end(); it++) {
        ...
        std::cout << it->first << " mu: " << mean << "\tsigma: " << stdev
                  << std::endl;
      }
    }
    ...
    void uploadKeypoints(std::vector<cv::KeyPoint> points,
      ...
      for (int j = 0; j < points.size(); ++j) {
        ...
        keypoints.at<float>(cv::cuda::SURF_CUDA::HESSIAN_ROW, j) =
            point.response;
        keypoints.at<int>(cv::cuda::SURF_CUDA::OCTAVE_ROW, j) =
            (int)(point.octave);
    ...
    void debugPrintDescriptors(cv::cuda::GpuMat &img) {
      ...
      for (int j = 0; j < res_host.rows; ++j) {
        ...
        std::cout << "Healthy: " << healthy << "\t\tZeros: " << zeros
                  << "\t\tExtrema: " << extremes << std::endl;
      }
    }
    ...
    void showCUDAMem() {
      ...
      //	printf("GPU memory usage: used = %f, free = %f MB, total = %f
